...
const filtered = computed(() => {
  ...
  if (search.value.trim()) {
    ...
    result = result.filter(t =>
      [t.title, t.description].some(v => String(v || "").toLowerCase().includes(s))
    );
  }
  ...
  if (filterAssigned.value) {
    result = result.filter(t => 
      t.assignees && t.assignees.some(assignee => assignee.id == filterAssigned.value)
    );
  }
...
function canUpdateTask(task) {
  ...
  return task.assignees && task.assignees.some(assignee => assignee.id === auth.user?.id);
}
...
async function updateTaskState(task, newState) {
  ...
  try {
    ...
    if (error.response?.status === 403) {
      Swal.fire("Error", "No tienes permisos para actualizar esta tarea", "error");
    } else {
...
<template>
  <div class="p-6">
    <div class="flex items-center text-sm text-slate-600 mb-4">
      <router-link to="/projects" class="hover:text-indigo-600">Proyectos</router-link>
    ...
    <div v-if="project" class="bg-indigo-50 border border-indigo-200 rounded-lg p-4 mb-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-xl font-semibold text-indigo-900">{{ project.name }}</h1>
          <p v-if="project.description" class="text-indigo-700 text-sm mt-1">{{ project.description }}</p>
          <div class="flex items-center gap-4 text-indigo-600 text-xs mt-2">
            <span>{{ project.collaborators_count }} colaboradores</span>
            <span>{{ tasks.length }} tareas</span>
            <span class="px-2 py-1 bg-indigo-200 text-indigo-800 rounded">{{ project.status }}</span>
          </div>
        </div>
    ...
    <div class="bg-white border rounded-xl p-4 mb-6">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div>
          <select v-model="filterState" class="w-full border rounded-lg px-3 py-2 text-sm">
        ...
        <div>
          <select v-model="filterPriority" class="w-full border rounded-lg px-3 py-2 text-sm">
        ...
        <div>
          <select v-model="filterAssigned" class="w-full border rounded-lg px-3 py-2 text-sm">
    ...
    <div class="bg-white border rounded-xl overflow-hidden">
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-if="loading">
            <td colspan="6" class="p-6 text-center text-slate-500">Cargando tareas...</td>
          </tr>
          ...
          >
            ...
            <td class="p-3">
              <div class="max-w-xs">
                <span class="text-slate-700 text-sm">{{ getAssigneesNames(task) }}</span>
                <div v-if="task.assignees && task.assignees.length > 1" class="flex flex-wrap gap-1 mt-1">
                  <span 
          ...
          <tr v-if="!loading && filtered.length === 0">
            <td colspan="6" class="p-6 text-center text-slate-500">
              {{ tasks.length === 0 ? 'No hay tareas en este proyecto.' : 'No se encontraron tareas con los filtros aplicados.' }}
            </td>
          </tr>
        </tbody>
      </table>
